...
function renderNoticas(){

    let nacionales = document.getElementById("nacionales")
    let internacionales = document.getElementById("internacionales")
    ...
    noticias.forEach(noticia => {
        ...
        imagenNoticia.className = "imgNoticia"
        let descripcionNoticia = document.createElement("p")
        descripcionNoticia.className = "descripcionNoticia"
        let fechaNoticia = document.createElement("p")
        fechaNoticia.className = "fechaNoticia"

        tituloNoticia.innerText = noticia.titulo
        imagenNoticia.src = noticia.imgUrl
        descripcionNoticia.innerText = noticia.descripcion
        fechaNoticia.innerText = noticia.fecha

        divNoticia.appendChild(tituloNoticia)
        divNoticia.appendChild(imagenNoticia)
        divNoticia.appendChild(descripcionNoticia)
        divNoticia.appendChild(fechaNoticia)
             
        if(noticia.tipoNacional){
            nacionales.appendChild(divNoticia)
        }
        else if (!noticia.tipoNacional){
            internacionales.appendChild(divNoticia)
        }
